  ...
  const parseCSV = (csv: string): Medication[] => {
    ...
    return lines.slice(1).map((line, index) => {
      ...
      const getVal = (keys: string[], defaultIdx: number) => {
        const foundIdx = lowerHeaders.findIndex(h => keys.some(k => h.includes(k.toLowerCase())));
        if (foundIdx !== -1) return values[foundIdx] || '';
        return values[defaultIdx] || ''; 
      };
      return {
        id: `row-${index}-${Date.now()}`,
        name: getVal(['中文', '商品名(中)'], 1),
        englishName: getVal(['英文', '商品名(英)'], 2),
        scientificName: getVal(['學名', '成分'], 3),
        specification: getVal(['規格', '包裝', '劑量'], 4),
        location: getVal(['儲位', '櫃', '位置'], 0),
        description: '', 
      };
    }).filter(m => m.name !== '' || m.englishName !== ''); 
  };
  ...
  const syncFromGoogleSheets = async () => {
    ...
    try {
      ...
      if (fetchUrl.includes('docs.google.com/spreadsheets') && fetchUrl.includes('/edit')) {
         const parts = fetchUrl.split('/edit');
         fetchUrl = parts[0] + '/export?format=csv';
      }
      ...
      if (newMeds.length > 0) {
        ...
        setLastSynced(new Date().toLocaleString('zh-TW', { hour12: false }));
        setSyncStatus({ type: 'success', msg: `更新成功，已同步 ${newMeds.length} 項儲位` });
      }
    } catch (err) {
      setSyncStatus({ type: 'error', msg: '同步失敗，請檢查網址或網路' });
    } finally {
      setIsSyncing(false);
    }
  };
    ...
    <div className="flex flex-col h-screen max-w-md mx-auto bg-white text-slate-900 overflow-hidden relative font-sans border-x border-slate-100">
        ...
        {activeTab === AppTab.SEARCH && (
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="relative group">
              ...
              <input 
                type="text" 
                placeholder="輸入中英文藥名關鍵字..." 
                className="w-full pl-14 pr-12 py-5 bg-white border-2 border-slate-100 rounded-3xl focus:ring-8 focus:ring-[#004766]/5 focus:border-[#004766] outline-none transition-all text-xl font-black text-slate-900 placeholder:text-slate-300 shadow-sm"
                value={searchTerm} 
                onChange={(e) => setSearchTerm(e.target.value)} 
              />
              {searchTerm && (
                <button onClick={() => setSearchTerm('')} className="absolute right-5 top-5 p-1 bg-slate-200 text-slate-500 rounded-full"><X size={16} /></button>
              )}
            </div>
              ...
              {filteredMedications.length > 0 ? filteredMedications.map(med => (
                <div key={med.id} className="p-6 rounded-[35px] border border-slate-100 shadow-[0_8px_25px_rgba(0,0,0,0.02)] flex flex-col group active:scale-[0.98] transition-all bg-white">
                  ...
                  <div className="flex flex-wrap gap-2 mb-5">
                    {med.scientificName && (
                      <div className="flex items-center gap-1.5 text-blue-800 font-bold text-[11px] bg-blue-50 px-3 py-1 rounded-full">
                  ...
                  <div className="flex items-center gap-4 px-8 py-4 bg-[#004766] text-white rounded-2xl shadow-xl">
                    ...
                    <span className="text-5xl font-black tracking-tighter tabular-nums flex-1">{med.location}</span>
          ...
          <div className="space-y-6 animate-in fade-in duration-500">
            <div className="bg-white p-8 rounded-[40px] space-y-8 border border-slate-100 shadow-sm">
              <div className="flex items-center gap-4">
                 ...
                 <div>
                   <h2 className="font-black text-slate-900 text-2xl">雲端同步</h2>
                   <p className="text-slate-400 font-bold text-xs tracking-tight">連接 Google 試算表 CSV</p>
                 </div>
              </div>
              
              <div className="space-y-6">
                <div className="space-y-3">
                  <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest pl-2">試算表來源網址</label>
                  <input 
                    type="text" 
                    className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-50 rounded-2xl text-base font-bold outline-none text-slate-900 focus:border-[#004766] transition-all"
                    value={sheetUrl} 
                    onChange={(e) => setSheetUrl(e.target.value)} 
                  />
                  <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100 flex gap-3">
                     <Info className="text-blue-500 shrink-0" size={20} />
                     <p className="text-xs font-bold text-blue-700 leading-relaxed">請確認試算表已「發布到網路」並選擇「CSV」格式。</p>
                  </div>
                </div>
                ...
                {syncStatus && (
                  <div className={`p-5 rounded-2xl text-base font-black flex items-center gap-3 ${syncStatus.type === 'success' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' : 'bg-rose-50 text-rose-700 border border-rose-100'}`}>
            ...
            <div className="bg-[#004766] p-8 rounded-[40px] text-white space-y-4 shadow-xl relative overflow-hidden">
              ...
              <div className="space-y-4 relative z-10 text-sm font-bold opacity-90">
                <p>1. 點擊手機瀏覽器的「分享」選單。</p>
                <p>2. 選擇「加入主螢幕」，即可像 App 一樣快速啟動。</p>
                <p>3. 若要更新資料，請聯繫維護藥師更新 Google 試算表。</p>
              </div>
            </div>
            ...
            <div className="text-center pt-10 pb-10">
               <div className="flex flex-col items-center gap-2">
                 ...
                 <p className="text-lg font-bold text-[#004766] flex items-center gap-2">
                   <Heart size={18} className="fill-[#004766]" />
                   許文馨 藥師
                 </p>
                 <div className="h-1 w-10 bg-[#004766]/10 rounded-full mt-4"></div>
                 <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mt-2">Storage Locator System v4.0</p>
               </div>
            </div>
          </div>
        )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white/95 backdrop-blur-xl border-t border-slate-100 flex justify-around items-center px-10 pb-safe pt-4 z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.04)] rounded-t-[35px]">
        ...
        >
          ...
          <span className="text-[11px] font-black tracking-widest">資料維護</span>
        </button>
      </nav>
    </div>
  );
